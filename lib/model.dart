...
class Models extends Model {
  ...
  int get newNotificationLength {
    return _newNotificationLength;
  }
  ...
  List<MyCartItem> get myCart {
    return List.from(_myCart);
  }
  ...
  allPendingOrders() async {
    ...
    //     reference.doc().firestore.collectionGroup('myPendingOrders');
    // return ref.where('pending', isEqualTo: pending).snapshots();
  }
  ...
  Future<Stream<QuerySnapshot>> fetchMyNotifications(String myId) async {
    CollectionReference ref = FirebaseFirestore.instance
  ...
  Future addMyNotifications(
      String userId, String title, String body, String navigatePage) async {
    CollectionReference ref = FirebaseFirestore.instance
        .collection('usersNotifications')
        .doc(userId)
        .collection('myNotifications');
  ...
  Future addToMyPurchases({
    ...
    await reference.doc(productId).snapshots().forEach((element) async {
      ...
      // Map<String, dynamic> myPurchases = {
      //   'productId': productId,
      //   'title': title,
      //   'noOfOrder': 1,
      //   'address': address,
      ...
      await FirebaseFirestore.instance
        ...
        element.docs.forEach((result) {
          ...
          SendPushNotifications.sendNotifications(dispatcherToken, msg);
        });
      });
  ...
  Future<Map<String, dynamic>> signIn(String email, String password) async {
    ...
    try {
      if (email.contains('@')) {
        ...
        await users.where('number', isEqualTo: email).get().then((value) async {
          if (value.size > 0) {
            successful = true;
            print('$successful 1');
            value.docs.forEach((element) async {
